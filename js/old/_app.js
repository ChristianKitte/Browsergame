...
jQuery(function () {
        ...
        function drawCanvas() {
            ...
            while (fps > maxFps) {
                dateEndDraw = Date.now();
                fps = Math.round(1000 / (dateEndDraw - dateStartDraw));
            }
...
function setStatus(levelManager, player) {
    ...
    let lastSpeed = "Speed: " + player.curSpeed.toFixed(2);
...
class Player {
    ...
    */
    ...
    constructor(canvas, playerSprite) {
        ...
        this.curSpeed = 0;
        this.deltaForwardSpeed = 0.05;
        this.deltaBackwardSpeed = 0.05;
        this.maxForwardSpeed = 0.5;
        this.maxBackwardSpeed = -0.5;
...
class FlowerManager {
    ...
    constructor(canvas, flowerSprite, levelManager, showFlower, audioSource1, audioSource2) {
        ...
        this.defaultFlowerTextFont = '0.5em serif';
...
class WaterJet {
     ...
     * The constructor
     * @param canvas The canvas
     * @param x1 Upper left point X
     * @param y1 Upper left point Y
     * @param x2 Upper right point X
     * @param y2 Upper right point Y
     * @param x3 Lower right point X
     * @param y3 Lower right point Y
     * @param x4 Lower left point X
     * @param y4 Lower left point Y
     ...
     */
    constructor(canvas, x1, y1, x2, y2, x3, y3, x4, y4, length, width, directionX, directionY, speed, side, value, color) {
        this.canvas = canvas;
        this.origin = [];
        this.origin[0] = x1;
        this.origin[1] = y1;
     ...
     * @param y_up The upper side
     * @param y_down The The bottom side
     * @returns {boolean} True, if a collision takes place
     */
    hasCollision(x_left, x_right, y_up, y_down) {
        let collide = false;

        // seems to be the same code for all sides...
        if (this.side === sides.LEFT) {
            if (x_left >= this.origin[0] && x_right <= this.origin[2]) {
                if (y_up >= this.origin[1] && y_down <= this.origin[5]) {
                    collide = true;
                }
            }
        ...
        } else if (this.side === sides.ABOVE) {
            if (x_left >= this.origin[0] && x_right <= this.origin[2]) {
                if (y_up >= this.origin[1] && y_down <= this.origin[5]) {
                    collide = true;
                }
            }
        } else if (this.side === sides.BELOW) {
            if (x_left >= this.origin[0] && x_right <= this.origin[2]) {
                if (y_up >= this.origin[1] && y_down <= this.origin[5]) {
                    collide = true;
                }
...
class WaterJetManager {
    ...
    CreateWaterJets() {
        ...
        let curLength = parseInt((this.defaultMaxHeight - this.defaultMinHeight) * (ranArray[0] / 2 ** 32) + this.defaultMinHeight);
        let curWidth = parseInt((this.defaultMaxWidth - this.defaultMinWidth) * (ranArray[1] / 2 ** 32) + this.defaultMinWidth);
        let curSpeed = parseInt((this.defaultMaxSpeed - this.defaultMinSpeed) * (ranArray[2] / 2 ** 32) + this.defaultMinSpeed);
        let curValue = parseInt((this.defaultMaxValue - this.defaultMinValue) * (ranArray[3] / 2 ** 32) + this.defaultMinValue);
        ...
        if (left < this.trunksPerSide) {
            ...
            let curAngle = Math.PI / 2 + ((ranArray[4] / 2 ** 32) * Math.PI);
            let randomY = parseInt(this.canvas.height * (ranArray[5] / 2 ** 32));

            curVector[0][0] = Math.cos(curAngle) * curVector[0][0] - Math.sin(curAngle) * curVector[1][0];
            curVector[1][0] = Math.sin(curAngle) * curVector[0][0] + Math.cos(curAngle) * curVector[1][0];

            let x1 = 0 - curLength; // left top then clockwise
            let y1 = randomY;
            ...
            let x3 = 0;
            let y3 = randomY + curWidth;

            let x4 = 0 - curLength;
            let y4 = randomY + curWidth;

            let newTrunk = new WaterJet(this.canvas, x1, y1, x2, y2, x3, y3, x4, y4, curLength, curWidth, curVector[0][0], curVector[1][0], curSpeed, sides.LEFT, curValue, this.defaultColor);
            ...
            let x1 = this.canvas.width; // left top then clockwise
            let y1 = randomY;

            let x2 = this.canvas.width + curLength;
            let y2 = randomY;

            let x3 = this.canvas.width + curLength;
            let y3 = randomY + curWidth;

            let x4 = this.canvas.width;
            let y4 = randomY + curWidth;

            let newTrunk = new WaterJet(this.canvas, x1, y1, x2, y2, x3, y3, x4, y4, curLength, curWidth, curVector[0][0], curVector[1][0], curSpeed, sides.RIGHT, curValue, this.defaultColor);
            this.trunks.push(newTrunk);
            ...
            let curAngle = (Math.PI / 2) + (ranArray[4] / 2 ** 32) * Math.PI;
            let randomX = parseInt(this.canvas.width * (ranArray[5] / 2 ** 32));

            curVector[0][0] = Math.cos(curAngle) * curVector[0][0] - Math.sin(curAngle) * curVector[1][0];
            curVector[1][0] = Math.sin(curAngle) * curVector[0][0] + Math.cos(curAngle) * curVector[1][0];

            let x1 = randomX; // left top then clockwise
            let y1 = -curLength;

            let x2 = randomX + curWidth;
            let y2 = -curLength;

            let x3 = randomX + curWidth;
            let y3 = 0;

            let x4 = randomX;
            let y4 = 0;

            let newTrunk = new WaterJet(this.canvas, x1, y1, x2, y2, x3, y3, x4, y4, curLength, curWidth, curVector[0][0], curVector[1][0], curSpeed, sides.ABOVE, curValue, this.defaultColor);
            this.trunks.push(newTrunk);
            ...
            let x3 = randomX + curWidth;
            let y3 = this.canvas.height + curLength;

            let x4 = randomX;
            let y4 = this.canvas.height + curLength;

            let newTrunk = new WaterJet(this.canvas, x1, y1, x2, y2, x3, y3, x4, y4, curLength, curWidth, curVector[0][0], curVector[1][0], curSpeed, sides.BELOW, curValue, this.defaultColor);
            this.trunks.push(newTrunk);
        }
    }
    ...
    MoveWaterJets() {
        ...
        if (this.trunks.length > 0) {
            this.trunks.forEach(function (entry, index) {
                ...
                let x1 = 0; // left top then clockwise
                let y1 = 0;
                ...
                if (item.side === sides.LEFT) {
                    ...
                    x1 = item.origin[0] + divX;
                    y1 = item.origin[1] + divY;

                    x2 = item.origin[2] + divX;
                    y2 = item.origin[3] + divY;

                    x3 = item.origin[4] + divX;
                    y3 = item.origin[5] + divY;

                    x4 = item.origin[6] + divX;
                    y4 = item.origin[7] + divY;
     ...
     * @returns {boolean} True, if a collision takes place
     */
    detectCollision(x_left, x_right, y_up, y_down) {
        let hasColide = false;

        this.trunks.forEach((Item, index) => {
                if (Item.hasCollision(x_left, x_right, y_up, y_down)) {
...
class LevelManager {
    ...
    constructor(canvas, audioSource1, audioSource2) {
        ...
        this.remainungHours = 0;
        this.remainigMinutes = 0;
